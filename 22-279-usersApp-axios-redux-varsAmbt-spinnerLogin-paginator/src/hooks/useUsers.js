...
export const useUsers = () => {
    ...
    const { users, userSelected, visibleForm, errors, isLoading, paginator } = useSelector(state => state.users);                                    //250 del estado, vamos a seleccionar a los usuarios
    ...
    const getUsers = async (page = 0) => {                                                              //se obtiene la lista de los usuarios. //findAll() devuelve una promesa.
        try {
            const result = await findAllPages(page);                                                     //se usa el await para evitar la promesa.
            console.log(result);
