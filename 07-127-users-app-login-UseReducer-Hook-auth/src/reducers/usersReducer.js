            ...
            return state.map(u => {                                                                     //se implementa con el state y el map para modificar el usuario.    
                if(u.id === action.payload.id){                                                         //si el id del usuario es identico al id usuario q viene en el payload. si es === es pq lo encontró.
                    return {
                        ...
                        password : u.password                                                           //se pone la password que ya tenía el usuario. esto quiere decir que se modifica la información del usuario en el update,  pero no la password.  con respecto al id,  no se pone, ya q este viene hidden.
                    }                                                               
                }
